...
class Grid(val list: List<MutableList<Tile>>) {
    ...
    override fun toString() = list.joinToString("\n") {
        val entities = it.filterIsInstance<Entity>()
        it.joinToString("") {
            when {
                it == Wall -> "#"
                it == Air -> "."
        ...
        } + if (entities.isNotEmpty()) entities.joinToString(" ", prefix = " ") { (if (it.elf) "E" else "G") + "(${it.hp})" } else ""
    }
}
...
fun main(args: Array<String>) {
    timeMili {
        ...
        for (attack in generateSequence(4) { it + 1 }) {
